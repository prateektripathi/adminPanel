...
const Dashboard = () => {
  const { user } = useAuth();
  const { stats, addStaff, addProduct, refreshStats } = useData();
  const { notifications } = useNotifications();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formType === 'staff') {
        ...
        productFormData.append('category', formData.additional || 'General');
        productFormData.append('stock', 100);
        productFormData.append('price', 99.99);
        
        result = await addProduct(productFormData);
      }
      ...
      if (result.success) {
        ...
        alert(result.message || `Error adding ${formType}`);
      }
    } catch (error) {
  ...
  const StaffDashboard = () => (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div className="space-y-3">
          {notifications.filter(n => n.type === 'task').slice(0, 5).map(task => (
